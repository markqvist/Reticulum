...
Once you have done that, take a look at the [Interfaces](interfaces.md#interfaces-main) chapter
of this manual.
...
You can easily add `rnsd` as an always-on service by [configuring a service](#using-systemd).
...
For the full documentation on the rngit system, see the [Git Over Reticulum](git.md#git-main) chapter of this manual.
...
The `rnodeconf` utility allows you to inspect and configure existing [RNodes](hardware.md#rnode-main), and
to create and provision new [RNodes](hardware.md#rnode-main) from any supported hardware devices.
...
For information on how to make your interfaces discoverable, see the [Discoverable Interfaces](interfaces.md#interfaces-discoverable) chapter of this manual. The current section will focus on how to actually *discover and connect to* interfaces available on the network.
...
#### NOTE
You can verify the external lists you are subscribed to, and their contents, without importing them by using `rnpath -p`. See the [rnpath utility documentation](#utility-rnpath) for details on querying remote blackhole lists.
